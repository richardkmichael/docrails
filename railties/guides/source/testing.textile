...
Every Rails application you build has three environments: production, development, and test.

One place you'll find this distinction is in the +config/database.yml+ file. This YAML configuration file has three different sections defining three unique database setups:
...
For example, suppose you need to test your new +delete_this_user_and_every_everything_associated_with_it+ method.  Clearly, this should be run in an environment with experimental data.

If you destroy your test database, you can rebuild it from scratch by running +rake db:test:prepare+.

h4. Rails Sets up for Testing from the Word "Go!"

Rails creates a +test+ folder when create a Rails project using +rails new _application_name_+.  Listing the contents of this folder, we see:
...
The +unit+ folder should contain tests for models, the +functional+ folder contains tests for controllers, and the +integration+ folder contains tests that involve any number of controllers interacting. Fixtures organize test data (see below); they reside in the +fixtures+ folder. The +test_helper.rb+ file is code common to all tests, to keep them DRY.
...
_Fixtures_ is a fancy word for sample data. Fixtures populate your testing database with predefined data before your tests run. Fixtures are database independent and assume a single format: *YAML*.

You'll find fixtures under your +test/fixtures+ directory. When you run +rails generate model+ to create a new model, fixture stubs will be automatically created and placed in this directory.

h5. YAML

*YAML* formatted fixtures are a very human-friendly way to describe your sample data. These types of fixtures have the *.yml* file extension, for example +users.yml+.

A sample YAML fixture file:

<yaml>
# YAML comments begin with an hash symbol.
david:
...
Each fixture is given a name followed by an indented list of colon-separated key/value pairs. Fixtures are separated by a blank line. You can place comments in a fixture file by using the # character in the first column.

h5. ERB'in It Up

ERB allows you to embed Ruby code within templates. Each YAML fixture file is pre-processed by ERB when you load fixtures. Therefore, Ruby can help generate sample data.
...
tags is considered Ruby code. When this fixture file is loaded, the +size+ attribute of the three records will be set to 20/50, 20/2, and 20-69 respectively. The +brightest_on+ attribute will also be evaluated and formatted by Rails to be compatible with the database.

h5. Fixtures in Action

By default, Rails automatically loads all fixtures from the +test/fixtures+ folder for your unit and functional tests. Loading fixtures involves three steps:
...
Fixtures can also transform themselves into the form of the original class. Thus, you may access methods only available to that class.
...
This guide uses Rails _scaffolding_.  A _scaffold_ will create the model, a migration, controller and views for a new resource in a single operation. It will also create a full test suite following Rails best practices. Examples from this generated code will be supplemented with additional examples where necessary.
...
A line by line examination of this file will introduce you to Rails testing code and terminology.

<ruby>
require 'test_helper'
</ruby>

The file +test_helper.rb+ specifies the default configuration to run our tests. It is included by all tests, so methods added to this file are available to all your tests.
...
Any method defined within a +Test::Unit+ test case that begins with +test+ (case sensitive) is simply called a test. So, +test_password+, +test_valid_password+ and +testValidPassword+ all are legal test names. Each test is run automatically when the test case is run.
...
with the +test+ macro merely providing a more readable test name. However, you may still use regular method definitions.
...
Every test contains one or more assertions. The test passes when all the assertions are true.

h4. Preparing your Application for Testing

Before you can run your tests, you need to ensure the test database structure is current. For this you can use the following rake commands:
...
The +rake db:migrate+ command runs any pending migrations from the _development_ environment and updates +db/schema.rb+. The +rake db:test:load+ recreates the test database from the current +db/schema.rb+. On subsequent attempts, it is a good idea to first run +db:test:prepare+, as it first checks for pending migrations and warns you appropriately.
...
This will run all the +test_*+ methods from the test case. Note +test_helper.rb+ is in the +test/+ directory, hence this directory needs to be added to the load path using the +-I+ switch.
...
In the output, +F+ denotes a failure. You can see the corresponding trace shown under +1)+ along with the name of the failing test. The next few lines contain the stack trace followed by a message which mentions the actual value and the value expected by the assertion. The default assertion messages provide just enough information to help pinpoint the error. To make the assertion failure message more informative, every assertion provides an optional message parameter, as shown here:
...
Running this test shows the more helpful assertion message:
...
Now for this test to pass we can add a model level validation for the _title_ field.
...
Now, if you noticed, we first wrote a test which fails for a desired functionality, then we wrote some code which adds the functionality and finally we ensured our test passes. This approach to software development is referred to as _Test Driven Development_ (TDD).
